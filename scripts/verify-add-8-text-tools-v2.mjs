...
const component=fs.readFileSync("src/components/tools/ExpansionToolWorkspace.astro","utf8");
...
["All modes connected",expected.every(s=>component.includes(`"${s}"`)&&route.includes(`"${s}"`))],
["Bullet logic exists",component.includes('if(mode==="add-bullets-to-text"){')],
["Blank-line logic exists",component.includes('if(mode==="add-blank-lines-between-text"){')],
];
let failed=false;
for(const [name,ok] of checks){console.log(`${ok?"OK":"FAILED"}: ${name}`);if(!ok)failed=true}
if(failed)process.exit(1);
console.log("8 text tools verification passed.");
